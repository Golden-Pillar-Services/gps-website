export const en = {
  a11y: {
    skipToContent: 'Skip to main content',
  },

  nav: {
    home: 'Home',
    services: 'Services',
    about: 'About',
    projects: 'Projects',
    contact: 'Contact',
    ariaLabel: 'Main navigation',
  },

  cta: {
    getQuote: 'Get a Quote',
    whatsapp: 'WhatsApp Us',
    viewAll: 'View All',
    learnMore: 'Learn More',
    startProject: 'Start Your Project',
  },

  home: {
    heroTitle: 'Built on Precision.\nFinished with Pride.',
    heroSub:
      "Fitout, joinery, signage and facility services for Oman's most demanding spaces — delivered on time, to specification.",
    servicesTitle: 'What We Do',
    servicesSub: 'Four trades, one contractor — from first fix to final polish.',
    whyTitle: 'Why Golden Pillar',
    whyPoints: [
      {
        title: 'On-time delivery',
        body: 'Programmes are built to hold. We track every trade against the schedule and flag risk before it becomes delay.',
      },
      {
        title: 'One contractor, every trade',
        body: 'Fitout, joinery, signage and cleaning under a single point of accountability — no gaps between subcontractors.',
      },
      {
        title: 'Omani market knowledge',
        body: 'Local approvals, local suppliers, local labour law — we navigate Muscat construction the way it actually works.',
      },
    ],
    statsLabels: {
      years: 'Years in Business',
      projects: 'Projects Delivered',
      clients: 'Clients Served',
    },
    featuredTitle: 'Recent Work',
    featuredSub: 'A sample of spaces we have built, fitted and finished.',
    ctaBand: {
      title: 'Have a project in mind?',
      sub: "Tell us the scope and the deadline — we'll tell you how we get there.",
    },
  },

  servicesPage: {
    title: 'Our Services',
    sub: 'Four disciplines, one standard of finish.',
    scopeTitle: 'Scope of Work',
    processTitle: 'How We Work',
    galleryTitle: 'Recent Work',
  },

  footer: {
    tagline: 'Fitout, joinery, signage and facility services across Oman.',
    servicesTitle: 'Services',
    quickLinksTitle: 'Quick Links',
    contactTitle: 'Contact',
    rights: 'All rights reserved.',
  },

  contact: {
    title: 'Contact Us',
    sub: "Reach out with your project scope — we'll respond within one business day.",
    channelsTitle: 'Get in Touch',
    whatsappLabel: 'WhatsApp',
    phoneLabel: 'Phone',
    emailLabel: 'Email',
    addressLabel: 'Address',
    hoursLabel: 'Working Hours',
    crLabel: 'CR Number',
    mapTitle: 'Golden Pillar Services location map',
  },

  about: {
    title: 'About Golden Pillar',
    sub: 'Built by tradespeople, run on discipline.',
    storyTitle: 'Our Story',
    storyP1:
      'Golden Pillar Services started as a small fitout crew working commercial units across Muscat. The standard was simple: show up, measure twice, finish clean, hand over on the date promised.',
    storyP2:
      "That standard is still the whole business model. We've since added joinery, signage and facility cleaning under the same roof, so clients deal with one contractor instead of four — but the promise from day one is the promise now: precise work, delivered on schedule.",
    valuesTitle: 'What We Hold To',
    values: [
      {
        title: 'Quality',
        body: 'Every fixing, seam and finish is checked against spec before we call a job done.',
      },
      {
        title: 'Reliability',
        body: 'We commit to dates we can hold, and we communicate the moment anything threatens them.',
      },
      {
        title: 'Craft',
        body: 'Our joinery and signage teams treat detail work as the job, not an afterthought.',
      },
    ],
    approachTitle: 'How We Approach a Project',
    approachBody:
      'Every engagement starts with a site visit and an honest scope, not a sales pitch. From there we quote what the work actually requires, sequence trades to avoid downtime, and keep one project lead accountable from kickoff to handover.',
    timelineTitle: 'Our Journey',
  },

  projects: {
    title: 'Projects',
    sub: 'Fitout, joinery and signage work across Muscat and beyond.',
    filterAll: 'All',
    lightboxClose: 'Close',
    lightboxNext: 'Next',
    lightboxPrev: 'Previous',
  },

  notFound: {
    title: 'Page Not Found',
    body: "The page you're looking for doesn't exist or has moved.",
    backHome: 'Back to Home',
  },
};
